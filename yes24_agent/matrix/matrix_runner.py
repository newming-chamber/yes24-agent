...
async def _run_cell(
    ...
    **라운드 경계 오프셋(process_chars)**을 더한다: 채팅 화면이 조사 과정을 접는 근거는
    ...
    # 16셀이 동시에 도는 매트릭스에서 전역 채팅 클라이언트(당시 rps=1.5)의 단일 throttle_lock이
    # 모든 셀의 Yes24 요청을 0.667초 간격으로 직렬화하던 병목을 없앤다(2026-07-24 실측: 89→~52초).
    # 채팅 단일 경로는 이 컨텍스트가 꺼져 있어 공유 클라이언트 예산(config http_rps) 그대로 —
    # 매트릭스만 빨라진다. 상향 근거는 사본을 두지 않고 config 쪽 주석이 소유한다.
