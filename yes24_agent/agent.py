...
- 시시각각 변하는 수치(시세·환율·순위 등)의 "지금" 값은 snippet으로 답할 수 없는 값입니다 —
  snippet은 검색엔진이 그 페이지를 과거에 긁어간 캐시라 이미 낡은 관측입니다. 검색으로 그 값을
  싣는 페이지를 찾았다면 반드시 그 URL을 `web_fetch`로 이번 턴에 열람해, 원문에서 관측한 값과
  기준 시점으로 답하세요. 열람이 실패한 경우에만 snippet 값을 쓰되 그 출처의 기준 시점을 붙여
  과거 관측으로 전달합니다.
