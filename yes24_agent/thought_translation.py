...
#
# 앞 문장은 **맥락만** 준다 — 중의어가 엉뚱한 분야로 풀리는 오역이 실측됐다(book value→
# 장부 가치, Mystery Novels→소설을 구상하는 중, Classical Philosophy→철학을 고찰하는 중).
# 맥락을 '지시'로 쓰면(쇼핑 말투로 옮기라고 하면) 번역기가 문구를 짓기 시작해 진행형이
# 깨지고 원문에 없는 말이 붙었다(변형 4종 74건 A/B 실측) — 그래서 뒤 문장은 그대로 뒀다.
_TRANSLATE_INSTRUCTION = (
    "도서·상품 쇼핑을 돕는 AI 어시스턴트의 진행 단계 제목이며, "
    "뜻이 여럿인 말은 그 맥락으로 읽는다. "
