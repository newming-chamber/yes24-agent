...
class Settings(BaseSettings):
    ...
    starter_trend_topics: int = 20
    # 오늘자 생성이 없는지 확인하는 주기(초). 0이면 확인 루프를 돌지 않고 **그날 첫 요청**이
    # 트리거다(종전 동작). 루프를 두는 이유: "요즘 화제" 슬롯이 생기면서 그날 첫 방문자가
    # 어제 화제를 보는 것이 이 기능의 핵심을 훼손하게 됐다. 여러 워커가 함께 돌아도 실제
    # 생성은 하루 한 번뿐이다 — starter_runs의 (slot, run_date) PK가 선점을 가른다.
    # 이 값은 주기 확인 간격이자 **실패·부족 슬롯의 재시도 cooldown**이다(둘이 같은 설정인
    # 이유: 재시도도 주기 확인이 한다). 짧게 잡으면 재료가 마른 슬롯이 하루에 그만큼 모델을
    # 다시 부른다 — 600초면 슬롯당 144회다. 화제 슬롯처럼 바깥 사정으로 얇아지는 슬롯이
    # 있으므로 시간 단위로 둔다.
    starter_refresh_interval_s: int = 3600
    ...
    # 기획전 목록에서 읽을 최대 줄 수(0이면 시즌 슬롯을 만들지 않는다). 이 슬롯의 재료는
    # **사이트가 판단한 시즌**이다 — 수능·명절·계절을 달력으로 코드에 박으면 매년 썩는데,
    # Yes24는 그 판단을 기획전 제목·기간으로 이미 내려 두었다. 오늘 진행 중인 것만 남긴다.
    starter_event_limit: int = 40
    # 슬롯당 하루 생성 개수. 회전 폭(요청마다 슬롯별 1개 무작위)의 재료 수이자 구조화 출력의
    # min_items=max_items(슬롯 수 × 이 값) — 개수는 프롬프트 문구가 아니라 스키마가 강제한다.
    # 슬롯당 하루 생성 개수 = 회전 폭. 5면 한 화면에서 같은 문구가 자주 겹쳐 보였다(12회
    # 서빙에 같은 문구 3~4회). 재료가 허락하는 만큼 넓히되 요청 수는 재료 수에 맞춰 깎인다.
    # 화제 재료로 받아들일 기사의 **발행 최신성**(일). 최신성은 모델이 주장하는 사건 날짜가
    # 아니라 검색 결과에서 파싱한 사실이어야 한다 — 모델에게 "사건이 오늘인가"를 물으면
    # 날짜가 증명되는 문서만 남고, 그런 문서는 대개 매일 갱신되는 섹션 색인 페이지라
    # 지역 행사 보도자료만 걸린다(2026-09-14 실측: 화제 8건 중 4건이 지역 공연 공지,
    # 관련 도서 0권). 창은 "지금 사람들이 이야기하는 것"의 폭이며 좁히면 재료가 마른다.
    starter_trend_window_days: int = 14
    # 화제 하나당 모델이 추가로 낼 수 있는 **서점 검색어 수** 상한. 화제의 표시 이름과 함께
    # 번갈아 검색해 후보를 만들며, 상세 열람 예산은 fetch_many_max_items가 따로 자른다.
    starter_trend_book_queries: int = 2
    # 하루에 돌릴 **대상 슬롯 수**의 하한·상한. 슬롯 후보는 사이트가 주는 만큼 생기므로
    # (코너 9 + 분야 32) 전부를 매일 생성하면 모델 호출이 후보 수만큼 든다. 날짜로 창을
    # 굴려 이 범위 안에서 **크기까지 날마다 다르게** 고른다 — 목록도 개수도 고정이 아니다.
    starter_rotating_slots_min: int = 3
    starter_rotating_slots_max: int = 6
    # 자동 생성분을 활성 풀로 인정하는 기간(일). 서빙은 "오늘 생성 대상인 슬롯인가"가 아니라
    # **언제 만든 문장인가**로 판정한다 — 슬롯 명단으로 거르면 오늘 안 돈 슬롯의 멀쩡한
    # 문장이 통째로 사라지고, 후보가 수십 개인 회전에서는 그게 풀의 대부분이다. 회전 주기
    # (후보 수 ÷ 하루 슬롯 수)보다 넉넉해야 모든 슬롯이 풀에 남는다.
    starter_pool_days: int = 14
    ...
    # 생성 서브콜 상한(초). 모델 콜 하나에 적용된다 — 백그라운드 태스크라 서빙 지연에는
    # 얹히지 않고, 매달린 콜이 잠금을 붙들어도 마감은 finally가 보장한다. 40인 이유: 화제
    # 수집은 웹 검색을 함께 도는 그라운딩 콜이라 15초를 넘겨 실패했다(실측 TimeoutError).
    starter_timeout_s: float = 40.0
    ...
    # `FOR UPDATE SKIP LOCKED`로 나눠 잡으므로 중복 파기·중복 감사가 없고, 상한은 한 트랜잭션의
    # 잠금 시간(CASCADE로 events까지 지운다)을 짧게 끊는 용도다. 밀린 분량은 같은 주기 안에서
    # 배치를 반복해 비운다.
    session_purge_batch_size: int = Field(default=100, ge=1)
    ...
    # 브라우저가 API를 부를 수 있는 오리진. `*`+credentials 조합은 브라우저가 거부하므로
    # 쓸 수 없고, 그렇다고 하나씩 적으면 **빠뜨린 것이 조용히 막힌다** — 프론트 포트가 바뀌거나
    # 새 프리뷰 도메인이 생길 때마다 CORS 에러가 나고, 그 차단은 서버 로그에 안 남아 원인을
    # 찾기 어렵다(2026-09-02~03에 세 번 겪었다: 3002·3003·3010 누락, crema-test 누락).
    # 그래서 **패턴**으로 바꾼다 — 우리 조직 도메인(griplabs.io·yes24.com)의 모든 서브도메인과
    # 로컬 개발 주소(포트 무관)를 허용한다. 목록을 갱신하지 않아 새는 일이 구조적으로 없어진다.
    #
    # 넓히는 위험은 낮다: 인증은 CORS가 아니라 `x-api-key`가 하고(오리진이 허용돼도 키가 없으면
    # 401), 쿠키 인증이 아니라 헤더 인증이라 브라우저가 자동으로 자격증명을 실어 보내지 않는다.
    # 특정 오리진만 열어야 하면 `CORS_ORIGINS`(명시 목록)를 함께 쓰면 된다 — 둘은 OR다.
    cors_origin_regex: str = (
        r"^https://([a-z0-9-]+\.)*(griplabs\.io|yes24\.com)$"
        r"|^http://(localhost|127\.0\.0\.1)(:\d+)?$"
    )
    # 패턴으로 안 잡히는 예외 오리진만 여기 적는다(기본은 비어 있다).
