...
- **지금 이 사용자의 코드**는 **`GET /me/rbti`**가 준다(`{"code":"SEBF"|null}`). 첫 턴을
  보내기 전에 배지·선택기 초기값을 그릴 값이며, 서버가 매 턴 쓰는 그 조회 결과와 같다.
  유형이 없거나 조회 실패면 `null`이고 **오류가 아니다** — 둘을 가르지 않는다. 저장·수정
  경로는 없다(코드의 정본은 외부 RBTI API다). 이 값도 진입 시 1회면 된다.
...
## 6-0. 내 독서 성향 (`GET /me/rbti`)

진입 시 배지·선택기 초기값을 그릴 값이다. 인증은 `/chat/stream`과 같은 `x-api-key`를 쓴다.

```json
{ "code": "SEBF" }
```

- `code`가 `null`이면 유형이 없거나 조회하지 못한 것이다. 화면은 둘을 같게 처리한다 —
  그 사용자는 성향 없이 답을 받는다.
- 여기서 나오는 코드는 **실제로 적용될 코드**다. 형식이 깨진 값은 서버가 걸러 `null`로
  내보내므로, 배지를 그렸는데 답변엔 성향이 안 걸리는 어긋남이 생기지 않는다.
- 이 값을 화면이 바꾸고 싶으면 `/chat/stream`의 `rbti`로 **그 턴만** 덮어쓴다. 저장 API는
  없다.
- 응답을 기다리느라 첫 화면을 막지 않는다 — 외부 조회라 지연이 있을 수 있다.
